...
# Map env → storage/appconfig + expected subscription name + RG
case "$ENV" in
  dev)
    ...
    # Dev RG optional (read from env var)
    DEV_RG="${RESOURCE_GROUP_DEV:-}"
    ;;
  test)
...
# ---- Guard 2: resource group existence
if [ "$ENV" = "dev" ]; then
  if [ -n "$DEV_RG" ]; then
    if ! az group show --name "$DEV_RG" --query name -o tsv >/dev/null 2>&1; then
      echo "❌ Resource group '$DEV_RG' not found in '$CURRENT_NAME'. Aborting."
      exit 1
    fi
    echo "✅ Resource group '$DEV_RG' is present."
  else
    echo "ℹ️ No RESOURCE_GROUP_DEV set; skipping Dev RG verification."
  fi
else
  if ! az group show --name "$EXPECTED_RG" --query name -o tsv >/dev/null 2>&1; then
    echo "❌ Resource group '$EXPECTED_RG' not found in '$CURRENT_NAME'. Aborting."
    exit 1
  fi
